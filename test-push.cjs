...
(async () => {
  ...
  const fcmToken = 'dNvXR5c8ms5lfmYOyYRsQ_:APA91bEE0nm0VnSmDbdu2MEfT-sjLFjJRbMkqypmKcR6pygSIGepGR89-o6vQs3pPyjN7h-dU9eaJ3cAFXB2gB8z78Tna96Te9AZcauTspNMIX15tS5RTPk';
  const res = await fetch('https://fcm.googleapis.com/v1/projects/confession-roulette-a6b4b/messages:send', {
    method: 'POST',
    ...
    body: JSON.stringify({
      message: {
        token: fcmToken,
        notification: {
          title: 'Antigravity says hi! 👋',
          body: 'Push notifications are fully working!'
        }
      }
    })
  });
  console.log('FCM Send Result:', await res.text());
})();
